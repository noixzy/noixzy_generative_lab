...
function render(g,pal){ const G=RDG;
  ...
  const img=makeField(G,(x,y)=>{ const raw=Math.min(1,Math.max(0,RDA[x+y*G]-RDB[x+y*G])); const v=Math.min(1,raw*4.2);
...
function heightField(G){ const sG=RDG, out=new Float32Array(G*G);
  for(let j=0;j<G;j++) for(let i=0;i<G;i++){
    ...
    const shaped=Math.min(1,raw*4.2);
    const edge=Math.min(i,j,G-1-i,G-1-j)/(G*0.075);
    const m=Math.min(1,Math.max(0,edge));
    const edgeMask=m*m*(3-2*m);
    out[i+j*G]=shaped*edgeMask; }
  return out; }` },
