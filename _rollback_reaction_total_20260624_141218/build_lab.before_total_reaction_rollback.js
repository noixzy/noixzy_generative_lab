...
function setup(){
  ...
  // Dial back reaction_diffusion default heaviness: keep it still/light on load.
  // Shared speed/drift can make this sim re-render continuously and feel non-responsive.
  if(PIECE=="reaction_diffusion"){
    P.speed=0;
    P.drift=0;
    if(P.glow!==undefined) P.glow=Math.min(P.glow,0.12);
    setTimeout(()=>{
      try{
        const s=document.getElementById('p_speed'); if(s) s.value = P.speed;
        const vs=document.getElementById('v_speed'); if(vs) vs.textContent = (P.speed).toFixed(2);
...
function renderScene(){
  ...
  try{
    ...
    if(typeof heightField === 'function' && (Math.abs(P.extrude||0)>0.01||(P.height||0)>0.01)){ const G=130; const heights = heightField(G);
...
{ id:"reaction_diffusion", title:"reaction diffusion",
  system:[{k:"feed",label:"feed",min:0,max:1,step:.01,v:.45},{k:"kill",label:"kill",min:0,max:1,step:.01,v:.55},{k:"spots",label:"seed spots",min:0,max:1,step:.01,v:.4},{k:"pal",label:"palette",min:0,max:9,step:1,v:4},{k:"height",g:"extrude",label:"height",min:0,max:1,step:.01,v:0,rr:true},{k:"hvar",g:"extrude",label:"variation",min:0,max:1,step:.01,v:.5,rr:true},{k:"light",g:"extrude",label:"light",min:0,max:1,step:.01,v:.5,rr:true}],
  code:`
let RDA,RDB,RDG;
function build(){
  const G=RDG=140; let a=new Float32Array(G*G).fill(1),b=new Float32Array(G*G),a2=new Float32Array(G*G),b2=new Float32Array(G*G);
  ...
  for(let it=0;it<900;it++){ for(let y=1;y<G-1;y++)for(let x=1;x<G-1;x++){ const i=x+y*G; let la=-a[i],lb=-b[i];
...
{ id:"voronoi", title:"voronoi",
  system:[{k:"density",label:"density",min:0,max:1,step:.01,v:.4},{k:"relax",label:"relax",min:0,max:1,step:.01,v:.4},{k:"edgefill",label:"edge / fill",min:0,max:1,step:.01,v:.5},{k:"pal",label:"palette",min:0,max:9,step:1,v:4},{k:"height",g:"extrude",label:"height",min:0,max:1,step:.01,v:0,rr:true},{k:"hvar",g:"extrude",label:"variation",min:0,max:1,step:.01,v:.5,rr:true},{k:"light",g:"extrude",label:"light",min:0,max:1,step:.01,v:.5,rr:true}],
  code:`
let VPTS;
...
function heightField(G){
  const ns=map(P.clustering,0,1,3.5,0.12), n=floor(map(P.density,0,1,6,40));
  const out=new Float32Array(G*G);
  for(let j=0;j<G;j++) for(let i=0;i<G;i++) out[i+j*G]=noise(i/G*n*ns, j/G*n*ns);
  return out;
}
...
{ id:"truchet_b", title:"truchet // color",
  system:[{k:"density",label:"density",min:0,max:1,step:.01,v:.45},{k:"weight",label:"weight",min:0,max:1,step:.01,v:.4},{k:"clustering",label:"clustering",min:0,max:1,step:.01,v:.5},{k:"pal",label:"palette",min:0,max:9,step:1,v:4},{k:"height",g:"extrude",label:"height",min:0,max:1,step:.01,v:0,rr:true},{k:"hvar",g:"extrude",label:"variation",min:0,max:1,step:.01,v:.5,rr:true},{k:"light",g:"extrude",label:"light",min:0,max:1,step:.01,v:.5,rr:true},{k:"speed",g:"motion",label:"speed",min:0,max:1,step:.01,v:.2}],
  code:`
function build(){}
function render(g,pal){ const n=floor(map(P.density,0,1,6,40)), cs=Math.min(g.width,g.height)/n;
  const cols=Math.ceil(g.width/cs), rows=Math.ceil(g.height/cs);
...
{ id:"cellular_erosion", title:"cellular erosion",
  system:[{k:"fill",label:"fill",min:0,max:1,step:.01,v:.48},{k:"iterations",label:"iterations",min:0,max:1,step:.01,v:.5},{k:"grain",label:"grain",min:0,max:1,step:.01,v:.4},{k:"pal",label:"palette",min:0,max:9,step:1,v:4},{k:"height",g:"extrude",label:"height",min:0,max:1,step:.01,v:0,rr:true},{k:"hvar",g:"extrude",label:"variation",min:0,max:1,step:.01,v:.5,rr:true},{k:"light",g:"extrude",label:"light",min:0,max:1,step:.01,v:.5,rr:true}],
  code:`
